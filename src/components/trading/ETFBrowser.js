...
export default function ETFBrowser({ onAddToOrder }) {
  ...
  // Apply "only with prices" filter
  const displayETFs = onlyWithPrices
    ? filteredETFs.filter(etf => {
        const md = marketData?.[etf.symbol];
        return md && (md.last || md.bid || md.ask);
      })
    : filteredETFs;
    ...
    <div className="bg-[#FEFEFE] border border-[#E8E8E6] rounded-xl overflow-hidden shadow-[0_2px_8px_rgba(45,52,54,0.06)]">
      ...
      <div className="relative">
        <div className="relative p-6 border-b border-[#E8E8E6]">
          ...
          {/* Results count + Price filter toggle */}
          <div className="mt-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[#636E72] text-sm">
                <span className="text-[#2D3436] font-semibold">{onlyWithPrices ? displayETFs.length : filteredCount}</span> ETFs gevonden
              </span>
            </div>
            <div className="flex items-center gap-3">
              {hasFilters && (
                <span className="text-[#B2BEC3] text-xs bg-[#F5F6F4] px-2 py-1 rounded hidden sm:inline">
        ...
        <table className="w-full">
          <thead className="bg-[#F5F6F4] sticky top-0 z-10">
            <tr>
              ...
              <th className="px-4 py-4 text-right text-xs font-semibold text-[#636E72] uppercase tracking-wider hidden lg:table-cell">
                Last
              </th>
              <th className="px-4 py-4 text-right text-xs font-semibold text-[#636E72] uppercase tracking-wider hidden lg:table-cell">
                Bid
          ...
          <tbody className="divide-y divide-[#E8E8E6]">
            {displayETFs.slice(0, 100).map((etf, index) => {
              const md = marketData?.[etf.symbol];
              return (
                <tr
                  key={etf.isin}
                ...
                >
                  ...
                  <td className="px-4 py-4 text-right hidden lg:table-cell">
                    {md?.last ? (
                      <span className="font-mono text-sm text-[#2D3436] font-medium">{md.last.toFixed(2)}</span>
                    ) : (
                  ...
                  <td className="px-4 py-4 text-right hidden lg:table-cell">
                    {md?.bid ? (
                      <span className="font-mono text-sm text-[#636E72]">{md.bid.toFixed(2)}</span>
                    ) : (
                      <span className="text-[#B2BEC3] text-xs">-</span>
                    )}
                  </td>
                  <td className="px-4 py-4 text-right hidden lg:table-cell">
                    {md?.ask ? (
                      <span className="font-mono text-sm text-[#636E72]">{md.ask.toFixed(2)}</span>
                    ) : (
                      <span className="text-[#B2BEC3] text-xs">-</span>
                    )}
                  </td>
        ...
        {displayETFs.length > 100 && (
          <div className="p-5 text-center bg-gradient-to-t from-[#F5F6F4]/50 to-transparent border-t border-[#E8E8E6]">
            <span className="text-[#636E72] text-sm">
              ...
              <span className="text-[#2D3436] font-medium">{displayETFs.length}</span> resultaten
            </span>
