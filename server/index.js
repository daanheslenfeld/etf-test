...
// In-memory store: { [customerId]: { email, name, lastSeen: Date } }
const _onlineUsers = {};
...
app.get('/api/admin/online-users', async (req, res) => {
  try {
    ...
    // "Online today": unique successful logins from login_log today
    ...
    // Deduplicate by email - keep most recent login per user
    ...
    res.status(200).json({
      ...
      online_now_count: onlineNow.length,
      online_today: onlineToday,
      online_today_count: onlineToday.length,
    });
  } catch (error) {
